<link rel="stylesheet" href="../styles/footer.css">

<footer>
  <div class="footer-container">
    <div class="footer-column">
      <h3>Quick Links</h3>
      <ul>
        <li><a href="../index.php">Home</a></li>
        <li><a href="../blog/index.php">Blog</a></li>
        <li><a href="../learn/index.php">Learn</a></li>
        <li><a href="../cheatsheet/index.php">Cheatsheet</a></li>
        <li><a href="../profile/index.php">Profile</a></li>
      </ul>
    </div>
    <div class="footer-column">
      <h3>About Us</h3>
      <ul>
        <li><a href="#">Background</a></li>
        <li><a href="#">F.A.Q.</a></li>
        <li><a href="../pages/privacy_policy.php">Privacy Policy</a></li>
        <li><a href="#">Open Source</a></li>
        <li><a href="../pages/feedback.php">Feedback</a></li>
      </ul>
    </div>
    <div class="footer-column">
      <h3>Contact Us</h3>
      <p>I am trying to spread the love of open source softwares.
        Knowledge should be free and open source!
      </p>
      <p>Email: sharafat@duck.com
        <br>GitHub: <a href="https://github.com/SharafatKarim/">@SharafatKarim</a> 
      </p>
    </div>
  </div>
  <div class="footer-bottom">
    <p>&copy; <?php echo date("Y"); ?> Created with ❤️ by Sharafat Karim. All rights reserved.</p>
  </div>
</footer>
</div>
</ul>